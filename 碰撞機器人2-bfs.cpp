...
int per[4];
bool used[4]; int dis[16][16];

void changeColor(char c){
	if(c=='W')SetConsoleTextAttribute(GetStdHandle(STD_OUTPUT_HANDLE), FOREGROUND_INTENSITY|FOREGROUND_RED|FOREGROUND_GREEN|FOREGROUND_BLUE);
	if(c=='R')SetConsoleTextAttribute(GetStdHandle(STD_OUTPUT_HANDLE), FOREGROUND_INTENSITY|FOREGROUND_RED);
	if(c=='Y')SetConsoleTextAttribute(GetStdHandle(STD_OUTPUT_HANDLE), FOREGROUND_INTENSITY|FOREGROUND_RED|FOREGROUND_GREEN);
	if(c=='G')SetConsoleTextAttribute(GetStdHandle(STD_OUTPUT_HANDLE), FOREGROUND_INTENSITY|FOREGROUND_GREEN);
	if(c=='C')SetConsoleTextAttribute(GetStdHandle(STD_OUTPUT_HANDLE), FOREGROUND_INTENSITY|FOREGROUND_GREEN|FOREGROUND_BLUE);
	if(c=='B')SetConsoleTextAttribute(GetStdHandle(STD_OUTPUT_HANDLE), FOREGROUND_INTENSITY|FOREGROUND_BLUE);
	if(c=='P')SetConsoleTextAttribute(GetStdHandle(STD_OUTPUT_HANDLE), FOREGROUND_INTENSITY|FOREGROUND_RED|FOREGROUND_BLUE);
}
				...
				if((j&3)==2){
					...
					if(gx[g]==i&&gy[g]==j>>2){
						for(; k<4; ++k)if(robot[k]==point(i, j>>2)){
							if(k==id)changeColor('Y');
							cout<<'Y';
							if(k==id)changeColor('W');
							break;
						}
						if(k==4)cout<<'X';
						k=5;
					...
					for(; k<4; ++k)if(robot[k]==point(i, j>>2)){
						if(k==id)changeColor('G');
						else if(!used[k])changeColor('R');
						cout<<k;
						changeColor('W');
						break;
					}
					if(k==4)cout<<(dis[i][j>>2]<1?' ':(char)(64+dis[i][j>>2]));
				}
				else cout<<' ';
			}
			else cout<<(r[i][j>>2]=='l'?'|':' ');
...
point nxtPos(point q, int d){
	point p=d==0?point(1, 0):d==1?point(0, 1):d==2?point(-1, 0):point(0, -1);
	for(;;q+=p){
		for(int i=0; i<4; ++i)if(used[i]&&q+p==robot[i])return q;
...
void bfs(){
	for(int i=0; i<16; ++i)for(int j=0; j<16; ++j)dis[i][j]=-1;
	queue<point> q; used[id]=0, q.push(robot[id]), dis[q.front().x][q.front().y]=0;
	while(!q.empty()){
		for(int i=0; i<4; ++i){
			point p=nxtPos(q.front(), i);
			if(dis[p.x][p.y]==-1)dis[p.x][p.y]=dis[q.front().x][q.front().y]+1, q.push(p);
		}
		q.pop();
	}
	makeMap(), used[id]=1;
...
Have fun!\n", system("PAUSE"), system("cls"),
	...
	while(1){
		...
		s="JKL ", s[3]=186;
		for(int i=0; i<4; ++i)if(i!=id&&key(s[i]))id=i, bfs();
		...
		if(key('H'))bfs();
		s="UIOP";
		for(int i=0; i<4; ++i)if(key(s[i])){used[i]=!used[i], bfs(); while(key(s[i]));}
	}
}
